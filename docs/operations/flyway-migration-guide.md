...
| 수정 금지 | **이미 `main`에 머지된 `Vn` 파일은 절대 수정하지 않습니다.** Flyway는 적용된 파일의
체크섬을 기억하고 있어서, 내용을 바꾸면 다음 사람 로컬/운영에서 마이그레이션 자체가
실패합니다. 스키마를 더 고쳐야 하면 새 `Vn+1` 파일로 `ALTER`를 추가합니다 |

## 3. 새 마이그레이션 파일을 추가하는 절차

1. **엔티티를 새로 만들거나 필드를 바꾸는 PR에는 반드시 대응하는 `Vn` 파일을 같은 PR에
   포함합니다.** 엔티티만 추가하고 마이그레이션 파일을 빠뜨리면 `ddl-auto: validate`
   전환 이후에는 앱이 기동조차 되지 않습니다.
...
- [ ] 새 엔티티/컬럼 변경에 대응하는 `Vn` 파일을 같은 PR에 포함했는가
