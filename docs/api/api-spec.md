...
- 비고: 응답은 배열이며, 항목별 `extractedText`는 포함하지 않는다(목록 조회 용도 —
  ...
  확인할 수 있도록 노출). 목록은 `createdAt` 내림차순(최신순)으로 정렬한다. `type`
...
- 비고: 채점 기준 5개 세부항목(7/9 확정)으로 내부 계산 — 기술적정확성10 / 핵심내용충족도5
  ...
  채점 및 꼬리질문 생성 LLM 호출은 DB 트랜잭션 밖에서 수행하고, 호출 전후의 짧은
  트랜잭션에서 세션 상태와 중복 결과를 다시 확인한다.
...
- 비고:
  ...
  - 최종 LLM 호출 자체는 DB 트랜잭션 밖에서 수행한다. 호출 전 준비 단계와 호출 후 반영
    단계가 각각 세션을 잠그고 상태·기존 최초 점수를 재검증하므로 외부 호출 중 커넥션을
    점유하지 않으면서 결과 반영의 원자성을 유지한다.
