...
- 비고:
  ...
  - TO-BE 바로 다음 줄에는
    `※ 아래 수치는 답변 구조를 보여주기 위한 가상 예시이며, 실제 측정 결과가 아닙니다.`
    고지를 포함하고, 모든 가상 수치는 `[예: ...]`로 표시한다. 사용자 리포트에는
    `turn`, `expectedAnswer`, `모범답안`, `confirmedScore`, `루브릭` 같은 내부 용어를
    노출하지 않는다.
  - 서버는 위 네 섹션 순서, Strengths 불릿 2개, AS-IS/TO-BE와 고지 위치, 내부 용어
    비노출을 검증한다. 형식 이탈 시 LLM 응답 스키마 오류와 동일하게 최대 1회 재요청한다.
