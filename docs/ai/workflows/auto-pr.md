...
1. **이슈 생성 (필요한 경우)**
   ...
   - 당장 착수하지 않는 **백로그성 이슈**만 등록 시 담당자 미정을 허용합니다. 이때는
     `--assignee`만 빼고 만들되, 본문에 우선순위 낮음과 착수 절차를 반드시 적습니다.
     제목 접두사와 라벨은 이슈 종류(`[FEAT]`/`enhancement`, `[BUG]`/`bug`)를 그대로
     유지합니다 — 백로그라는 이유로 종류를 바꾸지 않습니다.
     - `gh issue create --title "[FEAT] ..." --body-file <채운 파일> --label enhancement`
       (`--assignee` 없음. 버그면 `[BUG]`/`bug` 그대로)
