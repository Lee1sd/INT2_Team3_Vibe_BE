...
`docs/archive/모델비교_테스트_작업지시서.md` Phase 1의 축소판. 이력서 10개 전체가 아니라 이력서 1(커머스,
...
| 판단 | 1차 거부조건(`docs/archive/모델_선정_기준.md`) 관점: 두 모델 다 탈락 사유 없음(JSON 자체는 결국 둘 다 정상 파싱됨). 다만 Sonnet 응답이 더 길어 현재 타임아웃(30s)/토큰상한(2048)으로는 재시도가 잦을 수 있음 — Sonnet 부분 채택 시 해당 단계만 타임아웃/토큰상한을 늘리는 별도 설정이 필요할 것으로 보임. 이력서 10개 전체·Phase 2는 아직 미실행 |
...
`docs/archive/모델비교_테스트_작업지시서.md` Phase 2의 축소판. 이력서 3개 전체가 아니라 이력서 1(커머스,
...
### 2-46. #164 Lv.2 점수 산정·최종 리포트 프롬프트 섹션 격리

| 항목 | 내용 |
|---|---|
| 목적 | Lv.2 질문 난이도와 80점 커트라인은 유지하되, STRICT/CTO 리포트 지시·이전 저점수·부정적 feedback이 현재 문항 점수에 중복 반영되는 경로를 프롬프트 수준에서 차단 |
| 접근 | 별도 LLM 호출이나 Java 운영 흐름 변경 없이 기존 최종 호출 1회 안에서 `1단계 — turn 5 점수 산정`과 `2단계 — 최종 커리어 리포트 작성` 섹션을 분리. 점수를 먼저 확정하고 리포트 작성 중 변경하지 않도록 명시 |
| expectedAnswer | 모든 항목을 충족해야 하는 정답·감점 체크리스트가 아니라 평가 참고 기준으로 정의. 질문 의도에 부합하는 동등 개념·타당한 대안을 인정하고 질문이 요구하지 않은 세부사항 누락만으로 감점하지 않도록 수정 |
| 페르소나 경계 | 문항별 점수와 feedback은 중립적으로 작성하고, `personaTone`과 CTO 역할은 `overallFeedback` 리포트의 말투·피드백 강도에만 적용 |
| 꼬리질문 | 원래 질문 범위에서 누락된 핵심 개념 또는 판단 근거 하나만 확인. 새로운 고난도 주제·정량 지표·복수 트레이드오프·극단적 예외를 추가하지 않고 expectedAnswer도 누락 핵심만 간결하게 생성 |
| 비용·계약 | 정상 세션 4회 호출, API/DTO/DB/점수 합산·커트라인 계약을 그대로 유지. 프로덕션 Java 코드 변경 없음 |
| 전후 실측 하네스 | 커머스 백엔드 이력서+DB 키워드에서 파생한 고정 4+1 질문, expectedAnswer, 사용자 답변으로 최초채점+최종채점을 10회 반복하는 비활성 수동 테스트 추가. 당시 최종 요청의 `previousEvaluations`에는 각 실행의 최초 점수·feedback이 들어가 최종 프롬프트 입력까지 완전히 고정된 통제 비교는 아니었음 |
| 기준선 보존 | 실험 시작 커밋 `f12989b`의 채점 프롬프트 3개를 테스트 리소스에 스냅샷으로 보존했으며 당시 Git blob과 바이트 단위 내용 일치를 확인. 같은 테스트 코드에서 `baseline`/`current`만 전환해 비교 |
| 기준선 실측 10회 | 최초채점 평균 **68.4/80**, 꼬리질문 평균 **16.0/20**, 최종 평균 **84.4/100**(80~89), 합격 **10/10**. 공급자 호출은 최초 11회·최종 21회로 총 32회. 동일한 준수 답변은 변경 전에도 사람 기준으로 충분히 통과 가능한 범위였음 |
| 수정 후 실측 10회 | 최초채점 평균 **70.3/80**, 꼬리질문 평균 **18.3/20**, 최종 평균 **88.6/100**(86~93), 합격 **10/10**. 기준선 대비 평균 **+4.2점**. 공급자 호출은 최초 10회·최종 20회로 총 30회 |
| 해석 | 동일 질문·답변을 사용한 확률 표본에서 최초답변은 **+1.9점**, 꼬리답변은 **+2.3점** 차이가 났다. 다만 실행별 최초 평가와 그에 따른 이전 평가 컨텍스트 편차가 포함되어 있어 **+4.2점을 프롬프트 변경의 순수 효과로 단정할 수 없다**. 기준선도 이미 10/10 합격했으므로 기존 68~76점 사례 전체가 구조적으로 불합격하는 것은 아니며, 질문·답변 난도와 표본 구성의 영향도 크다. 광범위한 사람 면접관 블라인드 평가 없이는 적정 보정인지 점수 인플레이션인지 확정할 수 없음 |
| 리포트 형식 관찰 | 기준선은 복잡한 최종 리포트 형식을 1차 응답에서 **0/10**, 수정 직후 **1/10**만 충족해 기존 1회 재시도/폴백이 작동했다. 리포트 전용 지시를 더 명확히 한 추가 3회에서도 **1/3**만 1차 통과(점수 89·87·93, 총 8회 호출)하여 유의미한 개선을 확인하지 못함. 점수와 리포트를 한 호출에 유지하는 최소 변경 제약에서는 남는 구조적 한계로 기록 |
| 최초 401 | 첫 수동 실행 1회는 로컬 설정의 자리표시자 값으로 호출되어 Anthropic HTTP 401로 추론 전에 거부됨. 이후 `.env`의 `LLM_ANTHROPIC_API_KEY`를 각 Gradle 자식 프로세스에만 주입해 재실행했으며 키 값은 출력·기록하지 않음 |
| 리뷰 후 하네스 보정 | 이후 비교부터는 최종 요청의 이전 평가 점수·feedback도 고정해 실행별 최종 프롬프트 입력을 동일하게 유지한다. 표본 수는 비용 방어를 위해 1~20으로 제한하고 API 키는 환경변수에서만 읽는다. 기존 수치는 재실행하지 않고 위 한계와 함께 보존 |

### 2-47. #164 최종 결론

이슈 #164 최종 결론 — PR #177(최용성)로 채점 프롬프트 개선(expectedAnswer를 체크리스트가 아닌 참고
기준으로 전환 등). 통제된 고정 질문/답변 10회 테스트에서 기준선도 이미 10/10 합격(평균 84.4)이었고,
수정 후 평균 88.6(+4.2)으로 개선됐으나 완전한 순수 효과로 단정은 불가(previousEvaluations 컨텍스트
오염, 사람 블라인드 평가 부재). 이전 68~76점 실패 사례들은 질문 난이도 편차(§12 기존 인지 리스크)의
영향이 컸을 가능성이 높음. 리포트 형식 1차 통과율은 여전히 낮아(0~1/10) 재시도/fallback 의존 구조 유지.

### 2-48. #170/#179 채점 입력 신뢰 경계와 실제 면접 기반 리포트 대체

| 항목 | 내용 |
|---|---|
| 문제 | 질문·지원자 답변·모범답안·이전 평가가 평문으로 삽입돼 명령형 답변과 정적 지시의 구조적 경계가 약했다. 실 Claude 리포트는 첫 응답 형식 통과가 0~1/10 수준이어서 재요청 뒤 고정 사과문이 사용자에게 노출될 수 있었다 |
| 검토 대안 | 프롬프트 지시만 강화, 검증기 완화, 리포트 전용 LLM 호출 분리, 서버 결정적 리포트 조립을 비교했다. 지시만 강화하면 10/10을 보장하지 못하고, 검증 완화는 내부 용어·가상 수치 방어를 약화하며, 별도 호출은 세션당 호출 예산과 지연을 늘려 제외했다 |
| 입력 경계 | 최초·최종 동적 입력 전체를 `<interview-data>` JSON 블록으로 통합하고 `<`, `>`, `&`를 유니코드 escape했다. 데이터 안의 역할 변경·규칙 무시·출력 변경 문장은 평가 대상 텍스트로만 취급하도록 system/user 양쪽에 동일 계약을 추가했다 |
| 리포트 가용성 | 기존 최대 1회 리포트 재요청과 최초 점수 보존은 유지한다. 재요청도 실패하거나 JSON 파싱이 실패하면 최초 1~4 질문·확정 피드백과 turn 5 질문·답변·피드백으로 서버가 4섹션 Markdown을 조립하고 고지를 붙인다. 고정 사과문은 운영 최종판정 경로에서 사용하지 않는다 |
| 자동 검증 | 종료 태그·역할 변경 문장을 포함한 Claude Mock 요청 본문에서 JSON 경계를 파싱해 원문 보존과 실제 종료 태그 1개를 확인했다. 리포트 형식이 10회 연속 두 번씩 실패하는 Mock 표본도 10/10 모두 실제 면접 컨텍스트 4섹션 리포트, 고정 fallback 0건, 최초 점수 보존을 확인했다 |
| 실 API 검증 | 기본 10표본·최대 20표본 수동 테스트를 운영 환경변수 키 계약으로 보강했다. 현재 작업 환경에는 API 키가 없어 실호출은 수행하지 않았으며, 유효 키가 있는 통합/배포 환경에서 `ClaudeCareerReportRealApiTest`를 실행하고 `api-usage-log.md`에 즉시 기록해야 한다 |
| 구조 결정 | 외부 API·DB·점수·정상 세션 4회 호출 계약은 유지한다. 교차 경계 결정은 ADR-025에 기록했다 |

### 2-49. #170/#179 배포 동일 경로 Lv.2 이력서 5종 실 API 검증

| 항목 | 내용 |
|---|---|
| 목적 | 합성 이력서 5종에 대해 적합 답변과 부적합 답변을 각각 한 번씩 입력한 Lv.2 10건에서 실제 채점 분리와 최종 피드백 가용성을 빠르게 검증 |
| 빠른 실행 방식 | UI에서 세션 10개를 순차 생성하지 않고, 이력서별 고정 질문 4개와 최저점 대응 꼬리질문 1개를 사용했다. 배포와 같은 `ScoringPromptTemplate` → `LlmInvocationService` → `LlmEvaluationResponseAdapter` → `JudgmentScoringService` 경로와 실제 Claude Haiku 4.5를 병렬도 2로 실행했다. 질문 생성의 비결정성·UI/DB 대기만 제외했으며 프롬프트, 스키마 검증, 최대 1회 재요청, 문항 점수 clamp, 최종 합산과 Lv.2 80점 판정은 운영 코드 그대로다 |
| 실행 시간·호출량 | 약 3분 20초. 최초채점 10회, 최종판정 기본 10회, 리포트 재요청 9회로 공급자 요청 총 29회. 최초채점 스키마 재요청은 없었다 |
| 채점 결과 | 적합 답변은 87·90·92·89·86점(평균 **88.8**), **5/5 합격**. 부적합 답변은 17·15·16·19·16점(평균 **16.6**), **0/5 합격**. 다섯 쌍 모두 적합 답변 점수가 부적합 답변보다 높았고 평균 격차는 72.2점이었다 |
| 사례별 결과 | 커머스 87/17, 결제 90/15, 물류 이벤트 92/16, 데이터 플랫폼 89/19, 클라우드 플랫폼 86/16(각각 적합/부적합 순) |
| 리포트 결과 | 최종 사용자 노출 리포트는 **10/10 모두** 실제 질문·답변·확정 피드백을 포함한 4섹션 형식을 통과했고 기존 고정 사과문은 0건이었다. 모델의 첫 리포트 형식 통과는 1/10, 재요청은 9건이었다. 재요청 뒤 모델 리포트가 통과한 2건을 포함해 모델 리포트는 총 3건, 재요청도 실패해 서버가 실제 면접 문맥으로 조립한 리포트는 7건이었다 |
| 형식 실패 원인 | TO-BE의 가상 수치를 대괄호 밖에 쓰거나 내부 표현 `turn`을 노출하는 패턴이 반복됐다. 따라서 원시 모델 형식 준수율 자체가 개선된 것은 아니며, 10/10 가용성은 결정적 서버 리포트 대체가 보장했다 |
| 배포 UI 확인 | 병렬 하네스 전 실제 배포 UI에서 합성 이력서 1건을 업로드하고 Lv.1 면접 1건을 완료해 로그인·이력서 업로드·실 면접 호출 경로가 작동함을 확인했다. 이후 사용자의 속도 우선 요청에 따라 나머지 10건은 위 병렬 방식으로 전환했다 |
| 한계 | 실제 배포 서버의 HTTP 세션 10개를 만든 E2E 부하 테스트는 아니다. 배포와 동일한 코드·모델·환경변수 계약을 로컬에서 직접 실행한 통합 실측이므로 인증, 영속화, 폴링까지 포함한 배포 E2E는 별도 확인 범위다 |
| 판정 | 적합/부적합 채점 분리와 사용자 피드백 10/10 가용성 요구는 충족했다. 원시 LLM 리포트 형식 안정성은 낮으므로 최대 1회 재요청 및 실제 면접 문맥 기반 서버 리포트를 제거하면 안 된다 |

### 2-50. PR #192 리뷰 수정 후 Lv.2 이력서 5종 실 API 재검증

| 항목 | 내용 |
|---|---|
| 목적 | 리포트 재요청 null·런타임 예외 점수 보존, 서버 조립 리포트 최종 안전망, 한글 내부 용어 `턴` 차단을 반영한 뒤 적합/부적합 채점 분리와 최종 피드백 10/10 가용성이 유지되는지 확인 |
| 실행 방식 | #2-49와 동일한 배포 코드 경로와 고정 질문·답변을 사용하고 적합 5건·부적합 5건을 병렬도 2로 실행. Claude Haiku 4.5, 실제 `ScoringPromptTemplate`·`LlmInvocationService`·judgment 점수 보정 경로 사용 |
| 실행 시간·호출량 | 약 3분 15초. 최초채점 10회, 최종판정 기본 10회, 리포트 재요청 9회로 공급자 요청 총 29회. 비용 상한 50회 이내 |
| 채점 결과 | 적합 답변은 80·89·90·89·91점(평균 **87.8**), **5/5 합격**. 부적합 답변은 17·15·20·22·17점(평균 **18.2**), **0/5 합격**. 모든 이력서 쌍에서 적합 답변이 부적합 답변보다 높았다 |
| 사례별 결과 | 커머스 80/17, 결제 89/15, 물류 이벤트 90/20, 데이터 플랫폼 89/22, 클라우드 플랫폼 91/17(각각 적합/부적합 순) |
| 리포트 결과 | 사용자 노출 리포트 **10/10**이 네 섹션과 가상 수치 고지를 통과했다. 모델 리포트 1건, 실제 면접 문맥 기반 서버 리포트 9건, 최소 안전 리포트 0건, 과거 고정 사과문 0건, `turn`/`턴` 내부 용어 노출 0건 |
| 추가 발견 | 한글 내부 용어 `턴`을 모든 부분 문자열에서 치환해 정상 기술 용어 `패턴`이 `패문항`으로 변형되고, `expectedAnswer가`가 `평가 참고 내용가`로 치환되는 화면 문구 회귀를 발견했다. 형식·점수 검증은 통과했지만 사용자 문장 품질 결함으로 판단해 독립 토큰 경계와 치환 후 조사 보정을 추가했다 |
| 판정 | 리뷰 수정 뒤에도 Lv.2 적합/부적합 분리와 최종 피드백 가용성 요구를 충족했지만, 화면 문구 회귀를 보정한 뒤 #2-51에서 최종 재검증한다 |

### 2-51. 화면 문구 회귀 보정 후 Lv.2 이력서 5종 최종 재검증

| 항목 | 내용 |
|---|---|
| 보정 | 한글 `턴`은 앞뒤가 한글이 아닌 독립 토큰일 때만 내부 용어로 차단해 `패턴`을 보존했다. `expectedAnswer` 치환 뒤 붙은 `가`/`는`/`를`은 각각 자연스러운 `이`/`은`/`을`로 보정했다 |
| 실행 방식 | #2-49/#2-50과 동일한 Claude Haiku 4.5 배포 채점 경로, 합성 이력서 5종, 적합 5건·부적합 5건, 병렬도 2 |
| 실행 시간·호출량 | 약 3분 15초. 최초채점 10회, 최종판정 기본 10회, 리포트 재요청 9회로 총 29회 |
| 채점 결과 | 적합 답변 88·86·91·90·85점(평균 **88.0**), **5/5 합격**. 부적합 답변 19·15·17·17·17점(평균 **17.0**), **0/5 합격** |
| 사례별 결과 | 커머스 88/19, 결제 86/15, 물류 이벤트 91/17, 데이터 플랫폼 90/17, 클라우드 플랫폼 85/17(각각 적합/부적합 순) |
| 리포트 결과 | 사용자 노출 리포트 **10/10** 정상. 모델 리포트 4건, 실제 면접 문맥 기반 서버 리포트 6건, 최소 안전 리포트 0건, 과거 고정 사과문 0건. `패문항`, 어색한 `평가 참고 내용가/는/를`, 독립 `turn`/`턴` 노출은 모두 0건 |
| 판정 | 리뷰 수정과 화면 문구 회귀 보정 뒤에도 적합/부적합 분리 및 실제 면접 기반 최종 피드백 10/10 요구를 충족했다 |

### 2-52. 조사 결합 내부 용어 차단 후 Lv.2 이력서 5종 최종 재검증

| 항목 | 내용 |
|---|---|
| 보정 | CodeRabbit 후속 리뷰에 따라 독립 `턴`뿐 아니라 `턴의`, `턴은`처럼 조사가 결합된 내부 용어도 차단·치환한다. 정상 기술 용어 `패턴`은 계속 보존한다 |
| 실행 방식 | #2-49~#2-51과 동일한 Claude Haiku 4.5 배포 채점 경로, 합성 이력서 5종, 적합 5건·부적합 5건, 병렬도 2 |
| 실행 시간·호출량 | 약 3분. 최초채점 10회, 최종판정 기본 10회, 리포트 재요청 7회로 총 27회 |
| 채점 결과 | 적합 답변 84·86·92·88·83점(평균 **86.6**), **5/5 합격**. 부적합 답변 20·15·17·17·17점(평균 **17.2**), **0/5 합격** |
| 사례별 결과 | 커머스 84/20, 결제 86/15, 물류 이벤트 92/17, 데이터 플랫폼 88/17, 클라우드 플랫폼 83/17(각각 적합/부적합 순) |
| 리포트 결과 | 사용자 노출 리포트 **10/10** 정상. 모델 리포트 5건, 실제 면접 문맥 기반 서버 리포트 5건, 최소 안전 리포트 0건, 과거 고정 사과문 0건. 조사 결합을 포함한 독립 `turn`/`턴`, `패문항`, 어색한 `평가 참고 내용가/는/를` 노출은 모두 0건 |
| 판정 | 최종 리뷰 보정 뒤에도 적합/부적합 점수 분리, 실제 면접 기반 최종 피드백 10/10 가용성, 사용자 출력 경계 요구를 모두 충족했다 |
...
- **방어로직**: 동적 채점 입력 JSON 신뢰 경계, JSON 스키마 검증, 최대 1회 재요청,
  실제 면접 컨텍스트 기반 4섹션 리포트 대체, 점수 클램핑
