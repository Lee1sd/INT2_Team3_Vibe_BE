...
| 전후 실측 하네스 | 커머스 백엔드 이력서+DB 키워드에서 파생한 고정 4+1 질문, expectedAnswer, 사용자 답변으로 최초채점+최종채점을 10회 반복하는 비활성 수동 테스트 추가. 질문 생성 랜덤성을 제거해 프롬프트 변경만 비교 |
...
| 해석 | 이번 고정 표본에서는 expectedAnswer의 체크리스트화와 꼬리질문 난도 확장을 제한하자 최초답변 **+1.9점**, 꼬리답변 **+2.3점**이 상승했다. 다만 기준선도 이미 10/10 합격했으므로 기존 68~76점 사례 전체가 구조적으로 불합격하는 것은 아니며, 질문·답변 난도와 표본 구성의 영향도 크다. 광범위한 사람 면접관 블라인드 평가 없이는 현재 +4.2점이 적정 보정인지 점수 인플레이션인지 확정할 수 없음 |
