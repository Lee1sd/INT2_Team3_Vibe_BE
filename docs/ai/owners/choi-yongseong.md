...
## ⚠️ 팀 확인이 필요한 항목 (구현 전에 확정할 것)

상세 근거는 `docs/requirements/open-questions.md`가 SSOT이며, 여기서는 최용성 작업에
직접 영향을 주는 항목만 요약합니다.

- **뱃지 Stage 매핑**: `docs/api/api-spec.md`의 `BG-001` 비고에 "Stage3/4 매핑은 원문
  (Lv.3/Lv.4 표기) 대비 한 칸 당겨 해석한 것 — 팀 확인 필요"라고 되어 있습니다.
  FR-09 기준으로는 가입직후=Stage1, Lv.1 해금=Stage2, Lv.2 해금=Stage3, Lv.3 해금(스트레치골)=Stage4입니다.
  이 해석대로 구현하되, PR에서 명시적으로 이 가정을 언급하세요.
- **진행도 게이지 비율**: `UM-001` 비고에 "3레벨×30%=90%인지 100%로 맞출지 팀 확인
  필요 → 30, 30, 40 채워서 100으로 맞출 예정"이라고 되어 있습니다. FR-05는 "+30%씩
  누적"이라고만 되어 있어 마지막 레벨의 증가량(30 vs 40)이 확정되지 않았습니다.
  확정되기 전까지는 상수로 분리해두고 하드코딩하지 마세요.
- **레벨 해금 범위**: 기획서 3장#4 본문은 "MVP 범위 내 단일 레벨(대리) 집중"이라고
  서술하지만, API 명세서 `IV-001`과 FR-05, `mvp-scope.md`의 `FEAT-09`는 모두 Lv.1+Lv.2
  2단계 해금을 전제로 합니다. **2단계 해금 구조로 구현**하세요(open-questions.md #5).
