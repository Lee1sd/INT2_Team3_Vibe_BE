...
- #164 Lv.2(과장) 채점 밸런스 검증 — 실측(임시 진단 로그) 기반으로 루브릭 2차 조정
  (tradeOffsAndExceptions 원복, technicalAccuracy 정밀 완화) 반영 완료. real API 최종
  재검증도 완료: 68점으로 80점 미달 지속. 세션 표본이 적고(n=1) 세션마다 질문 난이도
  편차가 섞여 rubric 효과만 분리 판단은 어려움 — 표본 확대 또는 ③ 영역(커트라인/배점)
  재검토 등 다음 방향은 팀 판단 필요 (`docs/ai/ai-experiment-log.md` #2-37~#2-40 참고).
