...
- 직접 삭제 후 원본 파일 정리에 실패하면 수동 재시도가 가능하도록 원본 위치 키를
  `resume_file_cleanup_tasks`에 제한적으로 보관한다. 애플리케이션 로그에는 원문 키를 남기지 않고
  비가역적인 짧은 식별자만 기록한다. 정리 완료 후 작업 레코드는 함께 삭제한다.
