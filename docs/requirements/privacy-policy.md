...
- 면접 답변 및 대화 기록
- 마이페이지 프로필 이미지(사용자가 직접 업로드, [ADR-018](../adr/ADR-018-user-profile-image-s3.md))
...
4. **프로필 이미지는 위 1~3과 다른 정책을 따른다.** 이력서 원본과 달리 파싱 대상이
   ...
   보관**하고 파싱-후-삭제 대상이 아니다([ADR-018](../adr/ADR-018-user-profile-image-s3.md)).
...
- 프로필 이미지: 사용자가 새 이미지로 교체하거나(이전 객체 삭제) `DELETE /api/users/me/photo`로
  ...
  자체가 없어지므로 사실상 접근 불가능해진다([ADR-018](../adr/ADR-018-user-profile-image-s3.md)).
...
- [ ] 회원 탈퇴 시 프로필 이미지 S3 객체 삭제를 시도하는가? (`UserService.withdraw()`,
      실패해도 DB 삭제는 진행 — [ADR-018](../adr/ADR-018-user-profile-image-s3.md))
