...
| 8 | 꼬리질문 이후 최종 합산 문항 수 | FR-04는 4문항·100점 합산을 명시했으나 기존 `IS-002b` 예시와 ADR-008 구현 설명은 최초 최저점 문항을 제외한 2개와 꼬리질문만 반환해 3문항·최대 75점이 되는 불일치가 있었음 | 최초 3문항 점수와 꼬리질문 점수를 모두 포함한다. 질문 생성 LLM이 꼬리질문·예상답변을 반환하면 interview가 최초 3개 쌍과 합쳐 `{1,2,3,4}`를 최종 채점에 전달하고, 네 문항 총점으로 80점 합격 여부를 판정 | `docs/api/api-spec.md`(`IS-002b`), `docs/requirements/functional-requirements.md`(FR-04), `docs/adr/ADR-008-evaluation-response-dto-split.md` | ✅ 확정(2026-07-13, FR-04 원기획 4문항 기준 재확인) |
| 9 | 모범답변(expectedAnswer) 생성 시점 · questions 테이블 부재 | FR-03 처리 로직 4는 "질문 생성 호출 시 모범답변을 함께 생성"한다고 서술하는데, FR-04 처리 로직 1은 "채점 호출 내에서 모범답변을 생성하고 저장 안 함(휘발성)"이라고 서술 — 모범답변 생성 시점·저장 여부가 두 문서에서 서로 다르게 기술됨. ERD에는 질문·모범답변을 저장할 테이블이 없어 FR-03대로 가려면 신규 테이블이 필요함 | FR-03이 맞다 — 모범답변은 질문 생성 호출(FR-03)에서 함께 생성해 저장하고, 채점 호출(FR-04)은 저장된 값을 재사용한다(재생성 아님). expectedAnswer는 채점 로직 내부에서만 쓰이고 API·화면에는 절대 노출하지 않는다(FR-03 처리 로직 5, FR-04 처리 로직 4와 일치). **테이블 키 설계는 2026-07-14 번복**: `questions` 테이블은 `{sessionId, questionId}` 복합 UNIQUE가 아니라 `messageId`(→`Message.id`) 단일 PK/FK로 저장한다 — 질문 본문(`questionText`)은 이미 `Message.content`에 있으므로 중복 저장하지 않고, `expectedAnswer`만 해당 질문 메시지에 1:1로 붙인다 | `docs/requirements/functional-requirements.md`(FR-03, FR-04), `docs/erd/entity-definition.md`, `docs/api/api-spec.md`(`IS-002`, `IS-002b`) | ✅ 확정(2026-07-13, 김한비, `{sessionId, questionId}` 복합 UNIQUE) → ⚠️ 2026-07-14, 김한비 판단으로 `messageId` 단일 PK/FK로 번복 (근거 문서/링크: 확인 필요 — 이슈 #26 코멘트에 후속 정리). FR-03/FR-04/ERD/api-spec(`IS-002`, `IS-002b`) 문서 정정 완료. Flyway 마이그레이션(V2) 포함 여부는 이건희 확인 완료(별도 트래킹, 이슈 #38) |
