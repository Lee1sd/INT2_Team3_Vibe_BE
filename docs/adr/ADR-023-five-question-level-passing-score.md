...
- V24 적용 전에 존재하는 3+1문항 면접 기록은 새 4+1문항 계약으로 무손실 변환할 수 없다.
  현재는 운영 전이며 기존 면접 데이터가 폐기 가능하다는 팀 결정에 따라, V24가 기존 면접
  세션과 그 하위 메시지·질문·문항점수·최종판정을 제거한다. 해당 사용자의 해금 상태는
  Lv.1·게이지 0으로 되돌리고 가입 시 지급되는 Stage1은 유지하되 Stage2 이상 뱃지는 제거한다.
...
- **기존 25점 문항을 20점으로 clamp하고 turn 4를 turn 5로 변경** — 기존 세션에는 최초
  4번 질문이 존재하지 않아 문항 구성과 총점·개별점수 합계가 어긋나므로 반려했다.
- **기존 기록에 채점 계약 버전을 추가해 보존** — 운영 데이터 보존에는 적합하지만
  InterviewSession과 AnswerScore의 버전 분기 및 구 계약 조회 지원이 필요하다. 현재는
  운영 전 개발 데이터가 폐기 가능하므로 MVP 범위 대비 복잡도가 커서 반려했다.
...
- V24 적용 시 기존 면접 이력과 그 결과로 생성된 진행도·Stage2 이상 뱃지는 초기화된다.
  사용자·이력서와 가입 시 지급된 Stage1 뱃지는 유지된다.
