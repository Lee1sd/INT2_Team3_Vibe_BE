...
- 2026-07-29: 위 규칙이 "수치·결론을 고쳐 쓰지 않는다"로만 적혀 있어, 같은 커밋에서
  `모델비교_테스트_작업지시서.md`의 지시 원문(3~10초 체감 기준, 원안 표 2개)을 "서식·집계
  재구성"이라는 명분으로 삭제·치환한 자기모순이 발생함. 규칙을 **"원문 줄은 삭제·치환하지
  않고 덧붙이기만 허용"**으로 재정의하고, 덧붙이는 표기 3종(상단 배너 / `[사후 추가]` 블록 /
  취소선+`[확정]`)을 명시함. 판정 기준을 "무엇을 고쳤는가"가 아니라 **"원문 줄이 그대로
  남아 있는가"**로 바꾼 것이 핵심 — 전자는 "이건 사실 왜곡이 아니라 서식"이라는 자기변호를
  항상 허용하지만, 후자는 diff로 기계적으로 판정된다. 해당 문서의 원문은 되돌리고 통합 표는
  `[사후 추가]`로 다시 붙임. 함께, 보관 문서 위치를 `docs/archive/`로 정하고 루트에 있는
  기존 4개는 별도 PR에서 이동하기로 함(내용 정정과 파일 이동을 한 diff에 섞지 않기 위함).

- 2026-07-29: `SHARED.md` §4-5의 "이슈를 만들 때 담당자를 반드시 지정"이 백로그성 이슈와
  충돌하는 것을 발견해 완화함. 리뷰 중 발견한 후속 정리 과제를 이슈로 남기려 할 때, 당장
  착수할 사람이 없으면 규칙상 등록 자체가 막혀 "기록하지 않고 넘어가는" 쪽으로 흐른다.
  강제 시점을 **등록 시점 → 작업 시작 시점**으로 옮기고, 우선순위 낮음으로 명시한 백로그성
  이슈는 등록 시 담당자 미정을 허용하도록 했다. 규칙이 막으려던 것은 "담당자 없이 작업이
  굴러가는 상태"이지 "담당자 없는 기록"이 아니었다.
- 2026-07-30: 위 2026-07-29 항목에서 "별도 PR에서 이동하기로" 한 보관 문서 4개를 실제로
  `docs/archive/`로 옮김(이슈 #188). 이동 커밋은 순수 rename만 담고 참조 경로 갱신은
  별도 커밋으로 분리해, 리뷰어가 "경로만 바뀐 것"과 "내용이 바뀐 것"을 구분할 수 있게 했다.
  보관 문서 내부의 상호 참조는 네 파일이 같은 디렉터리로 함께 이동해 그대로 유효하며,
  원문 보존 규칙에 따라 손대지 않았다.
