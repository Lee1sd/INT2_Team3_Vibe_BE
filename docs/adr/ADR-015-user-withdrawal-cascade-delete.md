# ADR-015 — 회원 탈퇴: DB `ON DELETE CASCADE`로 전체 즉시 삭제
...
`V10__cascade_delete_on_user_withdrawal.sql`에서 위 체인에 속한 FK 10개를
...
- `V10__cascade_delete_on_user_withdrawal.sql` — 실제 FK 재정의
