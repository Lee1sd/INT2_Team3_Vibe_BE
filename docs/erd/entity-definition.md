...
| --- | --- | --- | --- | --- |
| `User` | `id`, `googleId`, `email`, `name`, `profileImageKey`(nullable) | ④ 인증 | FR-06, FR-12, 이슈 #98 | `name` 수정 가능(FR-12). `profileImageKey`는 S3 object key만 저장(URL 아님) — 응답의 `photoUrl`은 매 요청 새로 생성하는 Presigned GET URL([ADR-018](../adr/ADR-018-user-profile-image-s3.md)) |
