...
- `JudgmentResult.totalScore`는 DB와 애플리케이션에서 0~100으로 강제합니다.
  `passed`는 연결된 세션의 레벨에 따라 Lv.1은 `totalScore >= 60`, Lv.2는
  `totalScore >= 80`이어야 합니다. 이 교차 테이블 불변식은 DB 단일 행 CHECK로 표현하지
