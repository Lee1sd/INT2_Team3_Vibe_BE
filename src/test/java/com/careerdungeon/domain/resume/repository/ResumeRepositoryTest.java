...
class ResumeRepositoryTest {
    ...
    @Test
    @DisplayName("사용자의 PROCESSING/DONE/FAILED 이력서만 createdAt 내림차순으로 조회한다")
    void findByUserIdOrderByCreatedAtDesc_returnsAllStatusesForUserInDescendingOrder() {
        ...
        List<Resume> results = resumeRepository.findByUserIdOrderByCreatedAtDesc(USER_ID);
        ...
        assertThat(results).extracting(Resume::getCreatedAt)
                .containsExactly(
    ...
    @Test
    @DisplayName("FAILED 이력서 재업로드 시 createdAt 변경이 DB에 저장되어 최신 항목으로 조회된다")
    void replaceUpload_persistsCreatedAtAndMovesResumeToFirst() {
        Instant failedCreatedAt = Instant.parse("2026-07-01T00:00:00Z");
        Resume failed = resume(USER_ID, ParseStatus.FAILED, failedCreatedAt);
        ...
        assertThat(updated.getCreatedAt()).isAfter(failedCreatedAt);
    ...
    private Resume resume(Long userId, ParseStatus status, Instant createdAt) {
