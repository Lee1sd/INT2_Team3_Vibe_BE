...
CREATE TABLE `users` (
    `id`    BIGINT    NOT NULL    AUTO_INCREMENT,
    `google_id`    VARCHAR(255)    NOT NULL    COMMENT 'Google OAuth2 sub 값',
...
ALTER TABLE `persona_config` ADD CONSTRAINT `PK_PERSONA_CONFIG` PRIMARY KEY (
    `id`
);

ALTER TABLE `answer_scores` ADD CONSTRAINT `PK_ANSWER_SCORES` PRIMARY KEY (
    `id`
);

ALTER TABLE `user_unlock_status` ADD CONSTRAINT `PK_USER_UNLOCK_STATUS` PRIMARY KEY (
    `user_id`
);
...
ALTER TABLE `resumes` ADD CONSTRAINT `PK_RESUMES` PRIMARY KEY (
    `id`
);
...
ALTER TABLE `interview_sessions` ADD CONSTRAINT `PK_INTERVIEW_SESSIONS` PRIMARY KEY (
    `id`
);
...
ALTER TABLE `judgment_results` ADD CONSTRAINT `PK_JUDGMENT_RESULTS` PRIMARY KEY (
    `id`
);
